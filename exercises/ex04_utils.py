...
def is_equal(list1: list[int], list2: list[int]) -> bool:
    """Returns True if every element at every index of the list is equal in both lists"""
    if len(list1) != len(
        list2
    ):  # have to have this because if they are different lengths, every index in each of the lists cannot equal eachother
        return False
    index: int = 0
